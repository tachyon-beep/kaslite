...
def main():
    ...
    try:
        ...
        args = parser.parse_args(
            ["grid", "--config", "examples/quick_sweep.yaml", "--parallel", "2"]
        )
        ...
        for config_file in sweep_configs:
            try:
                ...
                combos = (
                    len(config.get_grid_combinations()) if config.sweep_type == "grid" else "N/A"
                )
                print(
                    f"      📄 {config_file.name}: {config.sweep_type} sweep, {combos} combinations"
                )
        ...
        print(
            "   python -c \"from morphogenetic_engine.sweeps.config import load_sweep_config; config = load_sweep_config('examples/quick_sweep.yaml'); print(f'Grid size: {len(config.get_grid_combinations())}')\""
        )
