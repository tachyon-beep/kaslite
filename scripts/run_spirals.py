...
def create_moons(n_samples: int = 2000, moon_noise: float = 0.1, moon_sep: float = 0.5, input_dim: int = 2):
...
def create_clusters(cluster_count: int = 2, cluster_size: int = 500, cluster_std: float = 0.5, 
                   cluster_sep: float = 3.0, input_dim: int = 3):
    ...
    for i in range(cluster_count):
        ...
        if input_dim == 2:
            ...
            center = cluster_sep * np.concatenate([
                ...
                rng.standard_normal(input_dim - 2)
            ])
...
def create_spheres(sphere_count: int = 2, sphere_size: int = 500, sphere_radii: str = "1,2", 
                  sphere_noise: float = 0.05, input_dim: int = 3):
...
@torch.no_grad()
def evaluate(
    model: nn.Module, loader: DataLoader, criterion: nn.Module, device: torch.device = torch.device("cpu")
) -> tuple[float, float]:
...
def main():
    ...
    parser.add_argument(
        "--problem_type", choices=["spirals", "moons", "clusters", "spheres", "complex_moons"], default="spirals",
        help="Type of problem to solve"
    )
    parser.add_argument("--n_samples", type=int, default=2000, help="Total samples (split evenly)")
    parser.add_argument("--input_dim", type=int, default=3, help="Embedding/dimension for clusters and spheres")
    ...
    parser.add_argument("--cluster_sep", type=float, default=3.0, help="Distance between cluster centers")
    
    # Spheres-specific parameters
    parser.add_argument("--sphere_count", type=int, default=2, help="Number of concentric spherical shells")
    parser.add_argument("--sphere_size", type=int, default=500, help="Points per sphere shell")
    parser.add_argument("--sphere_radii", type=str, default="1,2", help="Comma-separated list of radii")
    parser.add_argument("--sphere_noise", type=float, default=0.05, help="Jitter magnitude on sphere surface")
