...
from morphogenetic_engine.datasets import CIFAR10_AVAILABLE, create_cifar10
...
class TestCIFAR10CLIIntegration:
    ...
    def test_cifar10_specific_arguments(self):
        ...
        test_args = ["test", "--problem_type", "cifar10", "--input_dim", "3072", "--batch_size", "128", "--hidden_dim", "512"]
...
class TestCIFAR10EndToEnd:
    ...
    def test_cifar10_dataloaders_creation(self):
        ...
        # Check label range
        assert (
            0 <= train_y.min() <= train_y.max() <= 9
        ), f"Training labels should be in [0,9], got [{train_y.min()}, {train_y.max()}]"
