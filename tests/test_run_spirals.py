...
from scripts.run_spirals import (
...
class TestCreateSpirals:
    ...
    def test_spiral_structure(self):
        ...
        for class_label in [0, 1]:
            ...
            ), f"Class {class_label} correlation: {correlation}"
...
class TestMainFunction:
    ...
    def test_main_new_arguments(self):
        ...
        with patch("sys.argv", ["run_spirals.py"] + test_args):
            with patch(
                "scripts.run_spirals.create_complex_moons"
            ) as mock_create, patch("scripts.run_spirals.BaseNet") as mock_net, patch(
                "torch.optim.Adam"
            ) as mock_optim, patch(
                ...
                mock_create.assert_called_once_with(
                    n_samples=2000, noise=0.1, input_dim=4
                )
...
class TestIntegration:
    ...
    def test_full_pipeline_mini(self):
        ...
        for epoch in range(3):
            avg_loss = train_epoch(
                model, train_loader, optimizer, criterion, seed_manager
            )
    ...
    def test_seed_lifecycle(self):
        ...
        seed_manager = SeedManager()
        seed = SentinelSeed(
            "test_lifecycle", 16, seed_manager, blend_steps=5, progress_thresh=0.3
        )
...
class TestHighDimensionalIntegration:
    ...
    def test_backward_compatibility(self):
        ...
        model = BaseNet(
            hidden_dim=32, seed_manager=SeedManager()
        )  # Should default to input_dim=2
        x = torch.randn(4, 2)
        output = model(x)
...
class TestNewDatasets:
    ...
    def test_create_spheres(self):
        ...
        X, y = create_spheres(
            sphere_count=3, sphere_size=30, sphere_radii="1,2,3", input_dim=4
        )
        assert X.shape == (90, 4)
        assert y.shape == (90,)
        assert set(y) == {0, 1}  # Binary classification due to modulo
    ...
    def test_dataset_validation(self):
        ...
        with pytest.raises(ValueError, match="Number of radii"):
            create_spheres(
                sphere_count=2, sphere_size=50, sphere_radii="1,2,3", input_dim=3
            )
...
class TestCLIDispatch:
    ...
    def test_problem_type_dispatch(self):
        ...
        for problem_type, expected_function in test_cases:
            ...
            with patch("sys.argv", ["run_spirals.py"] + test_args):
                with patch(
                    f"scripts.run_spirals.{expected_function}"
                ) as mock_func, patch("scripts.run_spirals.BaseNet") as mock_net, patch(
                    "torch.optim.Adam"
                ) as mock_optim, patch(
                    "builtins.open", create=True
                ):
    ...
    def test_irrelevant_flags_ignored(self):
        ...
        with patch("sys.argv", ["run_spirals.py"] + test_args):
            with patch("scripts.run_spirals.create_spirals") as mock_spirals, patch(
                "scripts.run_spirals.BaseNet"
