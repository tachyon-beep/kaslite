...
TRAINING_LOSS = Gauge(
    'kaslite_training_loss',
    'Current training loss',
    ['phase', 'experiment_id']
)

BEST_ACCURACY = Gauge(
    'kaslite_best_accuracy',
...
SEED_ALPHA = Gauge(
    'kaslite_seed_alpha', 
    'Current alpha blending value for each seed',
    ['seed_id', 'experiment_id']
)

SEED_DRIFT = Gauge(
    'kaslite_seed_drift', 
    'Interface drift per seed',
    ['seed_id', 'experiment_id']
)
...
KASMINA_PATIENCE = Gauge(
    'kaslite_kasmina_patience',
    'Patience threshold for Kasmina controller',
    ['experiment_id']
)

# Training Duration Metrics
EPOCH_DURATION = Histogram(
    'kaslite_epoch_duration_seconds',
    'Time taken to complete an epoch',
    ['phase', 'experiment_id']
)

EXPERIMENT_DURATION = Gauge(
    'kaslite_experiment_duration_seconds',
...
class PrometheusMonitor:
    ...
    def update_seed_metrics(self, seed_id: str, state: str, alpha: float = 0.0, 
                           drift: float = 0.0, health_signal: float = 0.0, 
